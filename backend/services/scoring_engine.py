...
Includes AI code detection heuristics based on commit message analysis.
...
class ScoringEngine:
    ...
    def score_profile(self, profile: dict[str, Any]) -> dict[str, Any]:
        ...
        ai_savviness = self._score_ai_savviness(profile)
        ...
        ai_analysis = self._analyze_ai_usage(profile)
    ...
    def _score_ai_savviness(self, profile: dict[str, Any]) -> int:
        """Score AI-savviness (0-100).

        Factors: AI tool mentions in repos, AI-related topics,
        ML/AI framework usage, co-author bot tags.
        """
        score = 0.0
        repos = profile.get("repos", [])

        # AI-related topics (max 30 points)
        ai_topics = {
        ...
        score += min(ai_topic_count / 5 * 30, 30)

        # AI framework languages (max 30 points)
        ai_languages = {"Python", "Jupyter Notebook", "R"}
        lang_names = {l["name"] for l in profile.get("languages", [])}
        ai_lang_count = len(lang_names & ai_languages)
        score += min(ai_lang_count / 2 * 30, 30)

        # AI repo names/descriptions (max 20 points)
        ...
        score += min(ai_repos / 5 * 20, 20)

        # Bonus for high repo count with AI focus (max 20 points)
        if len(repos) > 0:
            ai_repo_ratio = ai_repos / len(repos)
            score += ai_repo_ratio * 20
    ...
    def _analyze_ai_usage(self, profile: dict[str, Any]) -> dict[str, Any]:
        """Analyze AI tool usage based on available data.

        In the MVP, this uses heuristics. Future versions may
        analyze actual commit messages with user opt-in.
        """
        repos = profile.get("repos", [])

        # Estimate AI usage bucket based on scoring
        ai_score = self._score_ai_savviness(profile)
        if ai_score >= 60:
            bucket = "60_100"
        elif ai_score >= 30:
            bucket = "30_60"
        ...
        return {
            ...
            "note": "Based on public repository metadata. Opt-in commit analysis for higher accuracy.",
        }
